...
import re
import sys
sys.path.insert(0, '/home/phil/PycharmProjects/JellyPy')

from pyCIPAPI.interpretation_requests import get_interpretation_request_json, count_tiered_variants
...
def parser_args():
    ...
    parser.add_argument(
        '--token',
        help='CIP API Authentication token, if available',
...
def main():
    ...
    ir_json_v6 = get_interpretation_request_json(ir_id, ir_version,
                                                 ...
                                                 reports_v6=True, testing_on=parsed_args.testing
                                                 )

    # Check that there aren't any tier 1 or 2 variants in IR
    # num_tiered_variants = count_tiered_variants(ir_json_v6, reports_v6=True)
    # if num_tiered_variants['T1'] != 0 | num_tiered_variants['T2'] != 0:
    #     sys.exit("Cannot close case as tier 1 and/or tier 2 variants are present")
    ...
    response = post_cr(clinical_report=cr, ir_json_v6=ir_json_v6, testing_on=parsed_args.testing)
